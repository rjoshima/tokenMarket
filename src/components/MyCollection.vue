<template>
    <div style="margin: 20px 10px 50px 10px;" v-loading="loading">
      <md-card v-for="(token, key, index) in tokens" :key="index"> 
            <md-card-media>
            ...
            <md-card-actions>
              <md-button class="md-raised" @click="burnToken(token.id)">BURN</md-button>
              <md-button class="md-raised" @click="quitSoldToken(token.id, token.true_price)" v-if="token.sold">{{token.price}}eth stop sell</md-button>
              <md-button class="md-raised" @click="soldToken(token.id, token.true_price)" v-if="token.sold == false">{{token.price}}eth sell in Market</md-button>
            </md-card-actions>
  ...
  export default {
    ...
    methods: {
      async updatetokens() {
        try {
          this.tokens = []
          await this.instance.tokensOfOwner(this.account).then(async (r) => {
